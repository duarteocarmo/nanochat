...
#
# Run:
#   bash runs/educational_score_ablation_ptcore5.sh
#
...
run_experiment() {
    ...
    uvx hf upload "$HF_MODEL_REPO" "$checkpoint_dir" "$model_tag/checkpoints" --repo-type model --exclude "optim_*.pt" --commit-message "Add $model_tag base checkpoint"
    ...
    echo "Uploaded $model_tag checkpoint, tokenizer, and evaluation to hf://$HF_MODEL_REPO/$model_tag"
}
