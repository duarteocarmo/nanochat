...
# Chinchilla-style Portuguese d12 pretraining run.
# Trains a depth-12 base model at ~20 train tokens per scaling parameter:
# 4,200 * 524,288 = 2,202,009,600 tokens (~2.202B).
# Run as:
# bash runs/runptd12-chinchilla.sh

set -euo pipefail

export OMP_NUM_THREADS=1
export NANOCHAT_BASE_DIR="$HOME/.cache/nanochat-pt-d12-chinchilla"
...
HF_BASE_REPO_ID=duarteocarmo/variacoes-d12-chinchilla-base
HF_SFT_REPO_ID=duarteocarmo/variacioes-d12-chinchilla-sft

# Dataset shards.
DATASET_TOKENIZER_SHARDS=8
DATASET_TRAIN_SHARDS=52
...
# Base pretraining.
BASE_TRAIN_DEPTH=12
BASE_TRAIN_DEVICE_BATCH_SIZE=16
BASE_TRAIN_TOTAL_BATCH_SIZE=524288
BASE_TRAIN_TARGET_PARAM_DATA_RATIO=20
BASE_TRAIN_CORE_METRIC_EVERY=700
BASE_TRAIN_CORE_METRIC_MAX_PER_TASK=-1
BASE_TRAIN_EVAL_TOKENS=4194304
BASE_TRAIN_SAMPLE_EVERY=700
BASE_TRAIN_SAVE_EVERY=-1
BASE_TRAIN_WANDB_RUN=d12_pt_chinchilla_$RUN_TIMESTAMP
...
CHAT_SFT_WANDB_RUN=d12_pt_chinchilla_sft_$RUN_TIMESTAMP
...
torchrun --standalone --nproc_per_node="$NPROC_PER_NODE" -m scripts.base_train -- \
    ...
    --save-every="$BASE_TRAIN_SAVE_EVERY" \
    --model-tag="$MODEL_TAG" \
